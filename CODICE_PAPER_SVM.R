...
#GRAFICI----
matrice <- as.data.frame(matrice_confusione_svm$table)
...
process_and_predict <- function(raster) {
  ...
  predizioni <- predict(SVM, newdata = pixel_data)
...
risultati_drone_SVM <- data.frame(
  area = c("OrthoEL1", "OrthoEY1", "OrthoEY3", "OrthoEY4", 
                  ...
                  "OrthoWY3", "OrthoWY4"),
  erba_cm2 = c(644688.00, 866105.0, 1248530.5, 443430.2, 
           1467882, 2434547, 1682602.0, 1632655.50, 
           824009.25, 594856.5, 550179.2, 1542086, 
           4657698.5, 1231437.0, 879277.2, 890030.75, 
           1732856, 945925, 1815507, 1546805.0, 
           2085050.00, 1380886.0, 837607.50, 982795.2, 
           1045665.2, 792830.5),
  fiore_cm2 = c(8963.25, 385115.8, 268922.2, 1717116.0, 
            2657, 1950, 200239.5, 53037.75, 
            860.25, 1058426.0, 216021.8, 70725, 
            22.5, 259746.2, 394318.0, 42911.25, 
            13356, 104291, 456681, 801642.5, 
            168.25, 120250.2, 3574.25, 61182.0, 
            15803.5, 590848.0)
)

# Stampa del dataframe per verificare i dati
print(risultati_drone_SVM)
...
dati_campo <- dati_campo[!dati_campo$area %in% c("EY4", "GU2", "VA2", "WY4", "VA1", "PA2", "EY3"), ]
risultati_drone_SVM <- risultati_drone_SVM[!risultati_drone_SVM$area %in% c("EY4", "GU2", "VA2", "WY4", "VA1", "PA2", "EY3"), ]
...
risultati_drone_SVM$area <- gsub("Ortho", "", risultati_drone_SVM$area)

# Unisci le colonne usando dplyr
dati_congiunti_SVM <- left_join(dati_campo, risultati_drone_SVM[, c("area", "fiore_cm2")], by = "area")

# Verifica i risultati
head(dati_congiunti_SVM)

# Calcola la correlazione tra le due stime dei cm² dei fiori
correlazione_SVM <- cor(dati_congiunti_SVM$flower_surface_cm2, dati_congiunti_SVM$fiore_cm2)

# Stampa il coefficiente di correlazione
print(correlazione_SVM)


# Costruisci il modello lineare
modello_lineare_SVM <- lm(fiore_cm2 ~ flower_surface_cm2, data = dati_congiunti_SVM)

# Riassunto del modello per ottenere R^2 e i p-value
summary_modello_SVM <- summary(modello_lineare_SVM)

# Estrai il valore R^2
r_quadrato_SVM <- summary_modello_SVM$r.squared

# Estrai il p-value per la pendenza (flower_surface_cm2)
p_value_SVM <- summary_modello_SVM$coefficients[2,4]

# Stampa i valori R^2 e p-value
print(paste("R^2:", r_quadrato_SVM))
...
annotazione_testo_SVM <- paste("R^2 = ", round(r_quadrato_SVM, digits = 3),
                           "\np-value = ", p_value_format_SVM)
# Crea il grafico con ggplot2 e aggiungi l'annotazione
ggplot(dati_congiunti_SVM, aes(x = flower_surface_cm2, y = fiore_cm2, label = area)) +
  geom_point() +
  geom_smooth(method = "lm", col = "blue") +
  geom_text(check_overlap = TRUE, nudge_y = 0.05, hjust = 0.5, vjust = -0.5) +
  labs(x = "Dati di campo (cm²)", 
       y = "Dati da drone SVM (cm²)", 
       title = "flower abundance(field) vs flower abundance UAV (SVM)") +
  theme_minimal(base_size = 12) +
  theme(plot.title = element_text(hjust = 0.5)) +
  annotate("text", x = Inf, y = Inf, label = annotazione_testo_SVM, hjust = 1.1, vjust = 1, size = 5, colour = "blue")




#BEE ABUNDANCE/FLOWER AB SVM----

# Calcola la correlazione
correlazione_SVM <- cor(dati_congiunti_SVM$bee_abundance, dati_congiunti_SVM$fiore_cm2)
print(paste("Correlazione:", correlazione_SVM))

# Costruisci il modello lineare
modello_lineare_SVM <- lm(fiore_cm2 ~ bee_abundance, data = dati_congiunti_SVM)
...
ggplot(dati_congiunti_SVM, aes(x = bee_abundance, y = fiore_cm2)) +
  geom_point() +
  geom_smooth(method = "lm", color = "blue") +
  labs(x = "Abbondanza delle api", y = "Area dei fiori (cm²)", title = "Correlazione tra Abbondanza delle Api e Area dei Fiori") +
  annotate("text", x = Inf, y = Inf, label = sprintf("R² = %.3f\np-value = %.3f", r_quadrato_SVM, p_value_SVM), hjust = 1.1, vjust = 2, size = 5, colour = "blue")


#BEE SPECIES RICHNESS / FLOWER AB. _SVM----

# Calcolo della correlazione tra bee_species_richness e fiori_cm2
correlazione_richness_SVM <- cor(dati_congiunti_SVM$bee_species_richness, dati_congiunti_SVM$fiore_cm2)

# Costruzione del modello lineare per bee_species_richness contro fiori_cm2
modello_lineare_richness_SVM <- lm(fiore_cm2 ~ bee_species_richness, data = dati_congiunti_SVM)

# Riassunto del modello per ottenere R^2 e il p-value
summary_modello_richness_SVM <- summary(modello_lineare_richness_SVM)

# Estrazione del valore R^2
r_quadrato_richness_SVM <- summary_modello_richness_SVM$r.squared

# Estrazione del p-value per la pendenza (bee_species_richness)
p_value_richness_SVM <- coef(summary(modello_lineare_richness_SVM))["bee_species_richness", "Pr(>|t|)"]

# Stampa dei risultati
print(paste("Correlazione:", correlazione_richness_SVM))
print(paste("R^2:", r_quadrato_richness_SVM))
print(paste("p-value:", p_value_richness_SVM))

ggplot(dati_congiunti_SVM, aes(x = bee_species_richness, y = fiore_cm2)) +
  ...
  annotate("text", x = max(dati_congiunti_SVM$bee_species_richness, na.rm = TRUE), y = min(dati_congiunti_SVM$fiore_cm2, na.rm = TRUE), 
           label = paste("R² =", round(r_quadrato_richness_SVM, 3), "\np-value =", format(p_value_richness_SVM, scientific = FALSE)), 
...
modello_lineare_shannon_SVM <- lm(fiore_cm2 ~ bee_shannon, data = dati_congiunti_SVM)

# Riassunto del modello per ottenere R^2 e il p-value
summary_modello_shannon_SVM <- summary(modello_lineare_shannon_SVM)
r_quadrato_shannon_SVM <- summary_modello_shannon_SVM$r.squared
p_value_shannon_SVM <- coef(summary(modello_lineare_shannon_SVM))["bee_shannon", "Pr(>|t|)"]


# Creazione del grafico con nomi delle aree
ggplot(dati_congiunti_SVM, aes(x = bee_shannon, y = fiore_cm2, label = area)) +
  geom_point() +  # Disegna i punti
  geom_smooth(method = "lm", color = "blue") +  # Aggiunge la linea di regressione
  ...
  annotate("text", x = max(dati_congiunti_SVM$bee_shannon, na.rm = TRUE), y = min(dati_congiunti_SVM$fiore_cm2, na.rm = TRUE), 
           label = paste("R² =", round(r_quadrato_shannon_SVM, 3), "\np-value =", format(p_value_shannon_SVM, scientific = FALSE)), 
           hjust = 1, vjust = -1, color = "red", size = 5) +
  theme_minimal()
